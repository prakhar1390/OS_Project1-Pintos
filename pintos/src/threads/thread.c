...
thread_unblock (struct thread *t) 
{
  ...
  list_insert_ordered(&ready_list, &t->elem,
		      (list_less_func *) &comparePriority,
		      NULL);
 ...
 int depth = 0;
  ...
  while (lk && depth < DEPTH_LIMIT)
      ...
      if (!lk->holder)
	{
      ...
      if (lk->holder->priority >= cur->priority)
	{
	  return;
	}
...
// remove the thread holding the lock
void threadUnlock(struct lock *lock)
{
